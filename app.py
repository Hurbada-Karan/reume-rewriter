...
def generate_automated_audit(cv_text, jd_text, api_key):
    clean_key = api_key.strip()
    
    if not clean_key or not clean_key.startswith("gsk_"):
        return "⚠️ **API Key Missing:** Please enter a valid Groq API key (starting with `gsk_`) in the sidebar."
...
with st.sidebar:
    ...
    # Fallback to Streamlit secrets if available, otherwise use input
    secret_key = st.secrets.get("GROQ_API_KEY", "") if "GROQ_API_KEY" in st.secrets else ""
    GROQ_API_KEY = user_api_key.strip() if user_api_key.strip() else secret_key
    ...
    if not job_description.strip():
        ...
        with st.spinner("Processing NLP embeddings, analyzing skill gap, and auditing weaknesses..."):
            ...
            if len(cleaned_cv) < 50:
                ...
                audit_report = generate_automated_audit(raw_cv_text, job_description, GROQ_API_KEY)
